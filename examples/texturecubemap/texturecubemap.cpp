...
* 
* Changes by Julio Cesar Pescuite Goncalves Batista - juliopescuite@gmail.com
* 
* Changelog:
...
public:
	...
	{
		title = "Daddy is learning Vulkan";
	...
	void loadAssets()
	{
		...
		// Objects
		std::vector<std::string> filenames = { "vulkanscenemodels.gltf", "deer.gltf", "chinesedragon.gltf", "venus.gltf", "torusknot.gltf"};
		objectNames = { "Sphere", "Teapot", "Torusknot", "Venus", "torusknot"};
		...
		loadCubemap(getAssetPath() + "textures/cubemap_vulkan.ktx", VK_FORMAT_R8G8B8A8_UNORM, forceLinearTiling);
	}
	...
	virtual void render()
	{
		...
		std::chrono::duration<double> elapsed = std::chrono::high_resolution_clock::now() - m_StartTimePoint;
		if (elapsed.count() >= 12)
		{
			m_StartTimePoint = std::chrono::high_resolution_clock::now();
			if (models.objectIndex < models.objects.size()-1)
			{
				models.objectIndex += 1;
			}
			else
			{
		...
		camera.setRotation(glm::vec3(0, mCamRot+=0.01f, 0));
		camera.updated = true;
		buildCommandBuffers();
		updateUniformBuffers();
		draw();
